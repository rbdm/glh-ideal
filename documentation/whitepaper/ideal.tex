...
% \paragraph{Roles, Links and Consequence.} A legal relationship is said to exist when the actions, attributes or character of an object satisfies a set of \textit{legal conditions} specified in a source of law. The \texttt{Role} associated with an object is the finite combination of actions, character, attributes or other contributing factors which satisfy those conditions. A \texttt{Link} is a relationship generated by a \texttt{Role} which has an associated \texttt{Consequence} modifying any other linked object.
